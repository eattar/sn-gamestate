...
class PartAveragedTripletLoss(nn.Module):
    ...
    def forward(self, part_based_embeddings, labels, parts_visibility=None):
        ...
        part_based_pairwise_dist = self._part_based_pairwise_distance_matrix(
            part_based_embeddings.transpose(1, 0), squared=False
        )
        ...
        pairwise_dist = self._combine_part_based_dist_matrices(
            part_based_pairwise_dist, valid_part_based_pairwise_dist_mask, labels
        )

        # Edge case handling: identical labels
        # If all labels are identical, there are no valid positives/negatives for triplet loss
        unique_labels = torch.unique(labels)
        if len(unique_labels) == 1:
            return torch.tensor(0.0, device=labels.device, requires_grad=True)

        # Compute standard batch-hard triplet loss using the pairwise distance matrix
        loss = self._hard_mine_triplet_loss(pairwise_dist, labels, self.margin)
        if loss is None:
            return torch.tensor(0.0, device=labels.device, requires_grad=True)
